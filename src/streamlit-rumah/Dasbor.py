...
from utils.data_loaders import (
    ...
    dim_districts,
    fact_price,
    fact_listing_by_district,
    fact_price_by_district,
    fact_correlations,
    fact_price_ratio,
    load_geojson_bogor,
)
...
def derive_quick_stats():
    ...
    return {
        "top_district": df_fact_listing_by_district.sort_values(
            "listing_count", ascending=False
        ).iloc[0, 1],
        "median_price": df_fact_price["price"].median(),
        "mean_price": df_fact_price["price"].mean(),
        "median_price_pl": df_fact_price_ratio["price_per_land_area"].median(),
        "median_price_pb": df_fact_price_ratio["price_per_building_area"].median(),
    }
...
def main():
    ...
    ch_price_df = df_fact_price_by_district.sort_values(
        "price_median", ascending=False
    ).head(10)
    ...
    ch_price = px.bar(
        ...
        labels={"price_median": "Median Harga Rumah", "district": "Kecamatan"},
    )
    st.plotly_chart(ch_price)

    # ---- top 10 districts by price box plot
    top_10_district_by_listing = (
        df_fact_listing_by_district.sort_values("listing_count", ascending=False)
        .head(10)
        .index
    )
    ...
    ch_price_box = px.box(
        ...
        labels={"price": "Harga Rumah", "district": "Kecamatan"},
    )
    ...
    # derive the map data
    df_choro = (
        df_fact_price.merge(df_dim_districts, on="district_sk")
        .groupby("district_x")
    ...
    ch_choro = px.choropleth_mapbox(
        ...
        zoom=8,
        opacity=0.5,
        center={"lat": -6.6, "lon": 106.8},
        hover_data={"district": True, "count": True, "price": ":,.0f"},
    )
