...
def construct_features(input_features: dict) -> pd.DataFrame:
  features = {
      ...
      "daya_listrik":
          input_features.get("daya_listrik", 0),
      "jumlah_lantai":
          input_features.get("jumlah_lantai", 0),
      "lebar_jalan":
          input_features.get("lebar_jalan", 0),
      "carport":
          input_features.get("carport", 0),
      "dapur":
          input_features.get("dapur", 0),
      "ruang_makan":
          1 if "Ruang Makan" in input_features.get("fasilitas", []) else 0,
      "ruang_tamu":
          1 if "Ruang Tamu" in input_features.get("fasilitas", []) else 0,
  }
  ...
  for material in AVAILABLE_HOUSE_MATERIAL:
    ...
    features[key] = 1 if material in input_features.get("house_material",
                                                        []) else 0
