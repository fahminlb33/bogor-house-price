  ...
  with Session(engine) as tx:
    ...
    if "prediction_llm" in _response:
      ...
      tx.add_all([
          ...
          create_openai_usage(USAGE_TYPE_PREDICTION_PARAPHRASE,
      ...
      tx.add_all([
          ...
          create_openai_usage(USAGE_TYPE_RAG_EMBEDDING,
                              _response["rag_embedder"]["meta"], prompt_id),

          # phase 3b.2: returned documents
          *create_rag_documents(_response["rag_doc_returner"]["documents"],
                                prompt_id),

          # phase 3b.3: QNA response
          create_openai_response(_response["rag_llm"]["replies"][0], prompt_id),
...
def create_openai_response(contents: str, prompt_id: str):
  return OpenAIResponse(
      id=str(uuid.uuid4()), contents=contents, prompt_id=prompt_id)
