...
def main():
    ...
    # --- total listing
    col1.metric("Total Listing",
                df_fact_listing_by_district.loc[district_sk, "listing_count"])
    # --- median price
    ...
    col2.metric("Median Harga per Luas Bangunan (m^2)",
                ...
                format_percent(
                    percent_change(building_median, global_building_median)),
                delta_color="inverse")

    st.caption("Perbandingan dengan median dan rata-rata kota")

    # price histogram
    ch_price_hist = px.histogram(df_fact_price.loc[district_sk],
                                 x="price",
                                 nbins=20,
                                 marginal="box",
