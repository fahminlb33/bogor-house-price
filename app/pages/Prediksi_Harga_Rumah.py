...
def main():
  ...
  with st.form("predict_form", border=False):
    ...
    with col2:
      ...
      input_features["kamar_pembantu"] = st.number_input(
          'Kamar pembantu', step=1)
    ...
    input_features["house_material"] = st.multiselect('Material Bangunan',
  ...
  if submit_res:
    ...
    # show prediction
    st.markdown(
        '<div class="text-center">Hasil Prediksi', unsafe_allow_html=True)
    st.markdown(
