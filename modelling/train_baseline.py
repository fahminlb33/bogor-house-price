...
class TrainRandomForest(TrainerMixin):
    ...
    def get_tensorboard_logdir(self, prefix: str, fold_i: int) -> str:
        return os.path.join(self.output_dir, "logs", f"{prefix}_{fold_i + 1}")
    ...
    def load_data(self):
        ...
        self.multihot_cols.extend(
            [col for col in df.columns if col.startswith("tags_")])
    ...
    def train(self):
        ...
        df_scores.to_csv(f"{self.output_dir}/raw_regression_scores.csv",
                         index=False)
        ...
        rdf.to_csv(f"{self.output_dir}/summary_regression_scores.csv",
                   index=False)
