...
class TrainRandomForest(TrainerMixin):
	...
	def get_tensorboard_logdir(self, prefix: str, fold_i: int) -> str:
		return os.path.join(self.output_dir, f"{self.run_name}_logs", f"{prefix}_{fold_i + 1}")
	...
	def train(self):
		...
		os.makedirs(os.path.join(self.output_dir, f"{self.run_name}_logs"), exist_ok=True)
		...
		df_scores.to_csv(f"{self.output_dir}/{self.run_name}-raw_scores.csv",
						 index=False)
		...
		rdf.to_csv(f"{self.output_dir}/{self.run_name}-summary.csv",
				   index=False)
